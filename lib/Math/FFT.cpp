// Transform template:
// for(int b = 1; b < n; b <<= 1) {         // Bit 
//     for(int u = 0; u < n; u += 2 * b) {  // Upper
//         for(int l = 0; l < b; l++) {     // Lower
            
//         }
//     }
// }

// Basic SOS DP in case I ever forget:
// for(int b = 1; b < n; b <<= 1) {         // Bit 
//     for(int u = 0; u < n; u += 2 * b) {  // Upper
//         for(int l = 0; l < b; l++) {     // Lower
// 			// At this point A[i] is the answer when only the first b-1 bits 
// 			// can be changed to form a submask.
// 			// So we're going to change the current bit b and update the answer
//             int submask = u|l;
// 			int mask = submask|b;
// 			A[mask] += A[submask];
//         }
