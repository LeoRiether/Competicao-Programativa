// https://github.com/tfg50/Competitive-Programming/blob/master/Biblioteca/Graph/MaxFlow.cpp
...
public:
	...
	int addEdge(int from, int to, T cap, T other = 0) {
		edges[from].push_back(list.size());
		list.push_back(Edge(to, cap));
		edges[to].push_back(list.size());
		list.push_back(Edge(from, other));
        return list.size() - 2; // index of the added forward-edge
	}
